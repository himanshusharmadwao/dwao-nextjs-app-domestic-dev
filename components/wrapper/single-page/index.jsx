...
const SinglePageWrapper = ({ pageData, relatedCapabilities, type = 'capabilities' }) => {
        ...
        return (
            <div className='' key={item?.slug}>
                {type === 'partners' ? (
                    ...
                    linkHref={`${process.env.NEXT_PUBLIC_DWAO_DOMESTIC_URL}/partners/${item.slug === 'partners-india' ? "" : item.slug}`} />
                ) : (
                    <RelatedCard imageSrc={item?.thumbnail} linkTitle={item?.title}
                    linkHref={`${process.env.NEXT_PUBLIC_DWAO_DOMESTIC_URL}/${item.slug}`} />
                )}
            </div>
        )
    })
